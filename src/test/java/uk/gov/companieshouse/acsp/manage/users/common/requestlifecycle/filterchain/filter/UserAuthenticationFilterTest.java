...
class UserAuthenticationFilterTest {
    ...
    void setup(){
        ...
        acspMembersService = Mockito.mock( MembershipService.class );
    ...
    void doFilterInternalWithValidAcspRequestTests( final User user, final StorageModel membership, final String ericAuthorisedTokenPermissions, final String expectedOutcome ) {
        final var request = new MockHttpServletRequest();
        request.addHeader( "X-Request-Id", "theId123" );
        request.addHeader( "Eric-Identity", user.getUserId() );
        request.addHeader( "Eric-Identity-Type","oauth2" );
